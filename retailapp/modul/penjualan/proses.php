...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    foreach ($penjualan as $item) {
        $sql = "INSERT INTO detail_penjualan (id_penjualan, id_barang, jumlah, harga, total_harga) VALUES('$id_penjualan', '$item[idbarang]', '$item[jumlah]', '$item[harga]', '$item[totalharga]')";
        $mysqli->query($sql);
        ...
        $result = $mysqli->query($sql);
        $row = $result->fetch_assoc();
        $sql = "UPDATE persediaan SET stok_awal = '$row[stok_akhir]', stok_masuk = 0, stok_keluar = '$item[jumlah]', stok_akhir = '$row[stok_akhir]', stok_masuk = 0, stok_keluar = '$item[jumlah]', stok_akhir = '$row[stok_akhir]' - '$item[jumlah]', deskripsi_persediaan = 'Penjualan' WHERE id_barang = '$item[idbarang]'";
        $mysqli->query($sql);
    }
}
?>