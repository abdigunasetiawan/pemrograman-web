...
function resetPenjualan() {
  // hapus semua data dilocal storage
...
function updateTabelPenjualan() {
  ...
  for (var i = 0; i < penjualan.length; i++) {
    tabelPenjualan.row.add([i + 1, penjualan[i].namabarang, penjualan[i].jumlah, penjualan[i].harga, penjualan[i].totalharga]).draw(false);
  }
  hitungTotalBayar();
}
